...
namespace MyExpenses.Domain.core.MappingProfile
{
    public class ActivityMapperProfile :Profile
    {
        public ActivityMapperProfile()
        {
            ...
            CreateMap<Activity, ApiActivity>().ForMember(dest => dest.TransactionType, opt => opt.MapFrom(src => src.Transaction.TransactionType.ToString())).ForMember(dest=>dest.Amount, opt=>opt.MapFrom(src=>src.Transaction.Amount)).ForMember(dest => dest.Date, opt => opt.MapFrom(src => src.Transaction.Date));
