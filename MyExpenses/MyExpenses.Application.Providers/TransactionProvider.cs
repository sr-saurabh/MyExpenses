...
namespace MyExpenses.Application.Providers
{
    public class TransactionProvider : ITransactionContract
    {
        ...
        /// <returns></returns>
        public async Task<CreateActivity> CreatePersonalExpenses(CreateActivity expense)
        {
            var activity = _mapper.Map<Activity>(expense);
            ...
            var goal = _goalRepo.Search(c => c.CategoryId == expense.CategoryId && c.Year == expense.Date.Year && c.Month == expense.Date.Month).SingleOrDefault();
            if (goal == null)
            {
                goal = new()
                {
            ...
            var accountDetail = await _accountRepo.GetByIdAsync(expense.AccountId);
            var isAccountUpdated = await _accountRepo.UpdateBalanceAsync(expense.AccountId, accountDetail.Balance - amount);

            return _mapper.Map<CreateActivity>(activity);
        ...
        public async Task<ApiActivity> GetPersonalExpense(int id)
        {
        ...
        /// <returns></returns>
        public async Task<IEnumerable<ApiActivity>> GetPersonalExpenses(int userId)
        {

            var personalExpenses = _activityRepo.Search(pe => pe.AppUserId == userId).ToList();
            return _mapper.Map<IEnumerable<ApiActivity>>(personalExpenses);
        }
        ...
        /// <returns> </returns>
        public async Task<ApiActivityWithSummary> GetPersonalExpenses(int userId, PersonalExpenseFilter filter)
        {
        ...
        public async Task<bool> UpdatePersonalExpense(UpdateActivity expense)
        {
            ...
            var res = await _transactionRepo.UpdateAsync(t => t.Id == previousExpense.TransactionId, t => t.SetProperty(t => t.Amount, expense.Amount).SetProperty(p => p.Date, expense.Date).SetProperty(p => p.Date, expense.Date));
        ...
        public async Task<bool> DeletePersonalExpense(int activityId)
        {
            ...
            var accountDetail = await _accountRepo.GetByIdAsync((int)previousExpense.Transaction.TransactionType);
        ...
        public async Task<WeeklyExpense> GetWeeklyExpense(int appUserId, bool isCurrent)
        {
